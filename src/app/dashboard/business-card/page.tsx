...
import { CreditCard, Upload, Trash2, RefreshCw, FileText, Image as ImageIcon, AlertCircle, CheckCircle2, X } from "lucide-react";
import { useState, useEffect, useRef } from "react";
import { useBusinessesStore } from "@/store/businesses.api";
import type { BusinessCard } from "@/types";

export default function BusinessCardPage() {
    const { myBusinesses, myLoading, fetchMyBusinesses, fetchCard, uploadCard, replaceCard, deleteCard } = useBusinessesStore();
    ...
    const handleUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            setCard(result);
            setMessage({ type: "success", text: card ? "Business card replaced!" : "Business card uploaded!" });
        } catch (err) {
            setMessage({ type: "error", text: (err as Error).message || "Failed to upload business card" });
        } finally {
            setUploading(false);
    ...
    const handleDelete = async () => {
        ...
        try {
            ...
            setMessage({ type: "error", text: (err as Error).message || "Failed to delete business card" });
        } finally {
            setDeleting(false);
        }
    };
    ...
    if (!business) {
        return (
            <div className="flex h-64 items-center justify-center">
                <p className="text-gray-500 dark:text-gray-400">No business found. Create a business first.</p>
            </div>
        );
    }
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-gray-900 dark:text-white">Business Card</h1>
                ...
                <div className="flex gap-2">
                    ...
                    {card && (
                        <Button variant="outline" onClick={handleDelete} disabled={deleting} className="text-red-600 hover:text-red-700 border-red-300 hover:border-red-400">
                    ...
                    {card ? (
                        <div className="space-y-4">
                            <div className="flex items-center gap-3 text-sm text-gray-600 dark:text-gray-400">
                                ...
                                <span>Uploaded {new Date(card.createdAt).toLocaleDateString()}</span>
                            </div>
                            {card.fileType === "pdf" ? (
