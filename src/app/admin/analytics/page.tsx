...
export default function AnalyticsPage() {
  const { dashboardStats, dashboardLoading, businesses, reviews, fetchDashboard, fetchRevenueAnalytics, fetchBusinesses, fetchReviews } = useAdminStore();
  ...
  useEffect(() => {
    ...
    fetchCategories();
    fetchCities();
  }, [fetchDashboard, fetchRevenueAnalytics, fetchBusinesses, fetchReviews, fetchCategories, fetchCities]);
  ...
  const categoryDistribution = categories
    .map((category) => ({
      name: category.name,
      count: businesses.filter((e) => e.categoryId === category.id).length,
    }))
    .sort((a, b) => b.count - a.count);

  // City distribution from real data
  const cityDistribution = cities
    .map((city) => ({
      name: city.name,
      count: businesses.filter((e) => e.cityId === city.id).length,
    }))
    .sort((a, b) => b.count - a.count)
    .slice(0, 5);
            ...
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
                  {(ds?.totalBusinesses ?? businesses.length).toLocaleString()}
                </p>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {ds?.approvedBusinesses ?? 0} approved
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
                  {ds?.totalBusinesses ?? businesses.length}
                </p>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {ds?.pendingApprovals ?? 0} pending
                </p>
              </div>
          ...
          <CardContent className="p-6">
            <div className="flex items-center justify-between">
              <div>
                ...
                <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
                  {ds?.totalReviews ?? reviews.length}
                </p>
                <p className="mt-1 text-xs text-gray-500 dark:text-gray-400">
                  {ds?.totalPayments ?? 0} payments
                </p>
              </div>
                      ...
                      <div className="flex items-center">
                        ...
                        <span className="text-sm font-medium text-gray-900 dark:text-white">
                          {reviews.filter((r) => r.businessId === business.id).length > 0
                            ? (reviews.filter((r) => r.businessId === business.id).reduce((s, r) => s + r.rating, 0) /
                                reviews.filter((r) => r.businessId === business.id).length).toFixed(1)
                            : "N/A"}
                        </span>
                      </div>
                    </td>
                    <td className="whitespace-nowrap px-4 py-3 text-sm text-gray-500 dark:text-gray-400">
                      {reviews.filter((r) => r.businessId === business.id).length}
                    </td>
                  </tr>
                ))}
              </tbody>
